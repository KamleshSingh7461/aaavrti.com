...
// Standalone version of getShiprocketToken
...
async function shipOrderManual(orderId: string) {
    try {
        const order = await prisma.order.findUnique({
            where: { id: orderId },
            include: { shippingAddress: true, items: { include: { product: true } }, user: true }
        });
        ...
        if (token && order.shippingAddress) {
            try {
                ...
                try {
                    ...
                    console.warn('[SHIPROCKET] Failed to fetch pickup locations, using default "Primary"');
                }
                ...
                const payload = {
                    ...
                    order_items: order.items.map(item => ({
                        ...
                        sku: item.product.slug || "SKU-" + item.productId.slice(0, 5),
                        units: item.quantity,
                ...
                console.log('[SHIPROCKET] Creating order in Shiprocket...');
                ...
                console.log('[SHIPROCKET] Order creation response:', JSON.stringify(shipData, null, 2));

                if (!shipData.order_id || !shipData.shipment_id) {
                    throw new Error(shipData.message || JSON.stringify(shipData));
                }
                ...
                console.log('[SHIPROCKET] AWB assignment response:', JSON.stringify(awbData, null, 2));
                ...
                try {
                    ...
                    console.log('[SHIPROCKET] Pickup response:', JSON.stringify(pickupGenData, null, 2));
                } catch (e) {
                    console.warn('[SHIPROCKET] Pickup generation failed (non-critical)');
                }
                ...
                try {
                    ...
                    console.log('[SHIPROCKET] Manifest response:', JSON.stringify(manifestData, null, 2));
                } catch (e) {
                    console.warn('[SHIPROCKET] Manifest generation failed (non-critical)');
                }
        ...
        console.log(`\n[SHIPROCKET] ✅ Order ${orderId} shipped successfully!`);
...
// Main execution
async function main() {
    const orderNumber = process.argv[2] || 'a2bda2c6';
    ...
    try {
        ...
        const result = await shipOrderManual(order.id);

        if (result.success) {
            console.log('\n✅ SUCCESS!');
            console.log(`Tracking ID: ${result.trackingId}`);
            console.log(`AWB Code: ${result.awbCode || 'N/A'}`);
            console.log('\n📍 Next Steps:');
            console.log(`   1. Refresh Shiprocket: https://app.shiprocket.in/seller/orders/new`);
            console.log(`   2. Check Admin: http://localhost:3000/admin/orders/${order.id}`);
        } else {
            console.log(`\n❌ FAILED: ${result.error}`);
        }
